...
if (argc != 5) {
    fprintf(stderr, "Usage: %s <input_csv> <reject_threshold> <Hm> <output_csv>\n", argv[0]);
    return 1;
}

char output_filename[64];
strncpy(filename, argv[1], sizeof(filename));
nxsrej = atof(argv[2]);  // outlier rejection threshold
hm = atof(argv[3]);      // expected norm (Hm)
strncpy(output_filename, argv[4], sizeof(output_filename));
...
// hm = 0.569;
 ...
 printf("  {%9.5lf,%9.5lf,%9.5lf}};\r\n",A_1[6],A_1[7],A_1[8]);
    ...
    // *** NEW CODE: write code snippet to a header file ***
    {
        char header_filename[64];
        /* Copy the output filename (argv[4]) into header_filename */
        strncpy(header_filename, output_filename, sizeof(header_filename));
        /* Find the last dot in the filename and replace the extension with ".h" */
        char *dot = strrchr(header_filename, '.');
        if(dot != NULL) {
            strcpy(dot, ".h");
        } else {
            strcat(header_filename, ".h");
        }
        FILE *hp = fopen(header_filename, "w");
        if (hp != NULL) {
            fprintf(hp, "float B[3] = { %8.2lf, %8.2lf, %8.2lf };\n\n", B[0], B[1], B[2]);
            fprintf(hp, "float Ainv[3][3] = {\n");
            fprintf(hp, "  { %9.5lf, %9.5lf, %9.5lf },\n", A_1[0], A_1[1], A_1[2]);
            fprintf(hp, "  { %9.5lf, %9.5lf, %9.5lf },\n", A_1[3], A_1[4], A_1[5]);
            fprintf(hp, "  { %9.5lf, %9.5lf, %9.5lf }\n};\n", A_1[6], A_1[7], A_1[8]);
            fclose(hp);
            printf("\r\nCode snippet saved to header file: %s\n", header_filename);
        } else {
            printf("\r\nError: unable to create header file %s\n", header_filename);
        }
    }
